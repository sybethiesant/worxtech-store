...
function DomainSearch({ onAddToCart }) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)]">
      ...
      {/* Popular Extensions - Clean minimal display */}
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              ...
              <p className="text-slate-600 dark:text-slate-400">
                {pricing.length}+ domain extensions available. Search above to see pricing for any TLD.
              </p>
            </div>

            {/* Show top 8 popular TLDs in a clean horizontal layout */}
            <div className="flex flex-wrap justify-center gap-4">
              {pricing
                .sort((a, b) => {
                  // Prioritize common TLDs
                  const priority = ["com", "net", "org", "io", "co", "dev", "app", "ai"];
                  const aIdx = priority.indexOf(a.tld);
                  const bIdx = priority.indexOf(b.tld);
                  if (aIdx !== -1 && bIdx !== -1) return aIdx - bIdx;
                  if (aIdx !== -1) return -1;
                  if (bIdx !== -1) return 1;
                  return parseFloat(a.price_register) - parseFloat(b.price_register);
                })
                .slice(0, 8)
                .map((p) => (
                <div
                  key={p.tld}
                  className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 rounded-xl px-5 py-3 border border-slate-200 dark:border-slate-700"
                >
                  <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">.{p.tld}</span>
                  <span className="text-slate-900 dark:text-slate-100 font-semibold">
                    ${parseFloat(p.price_register).toFixed(0)}
                  </span>
